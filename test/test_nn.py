  ...
  def test_batchnorm2d(self, training=False, threed=False, track_running_stats=True):
    with Tensor.train(training):
      ...
      for sz in szs:
        ...
        with torch.no_grad():
          if threed:
            # Idk if this would piss him off...
            tbn = torch.nn.BatchNorm3d(sz, track_running_stats=track_running_stats).train(training)
          else:
            tbn = torch.nn.BatchNorm2d(sz, track_running_stats=track_running_stats).train(training)
        ...
        if track_running_stats:
          np.testing.assert_allclose(bn.running_mean.numpy(), tbn.running_mean.detach().numpy(), rtol=5e-3, atol=1e-6)
          np.testing.assert_allclose(bn.running_var.numpy(), tbn.running_var.detach().numpy(), rtol=5e-3, atol=1e-6)

  def test_batchnorm2d_nostats(self): self.test_batchnorm2d(False, track_running_stats=False)
  def test_batchnorm2d_training(self):
    self.test_batchnorm2d(True, track_running_stats=True)
    self.test_batchnorm2d(True, track_running_stats=False)

  def test_batchnorm3d(self):
    self.test_batchnorm2d(False, True, track_running_stats=False)
    self.test_batchnorm2d(False, True, track_running_stats=True)
  def test_batchnorm3d_training(self):
    self.test_batchnorm2d(True, True, track_running_stats=False)
    self.test_batchnorm2d(True, True, track_running_stats=True)

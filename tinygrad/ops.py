...
#         # Split the value if it contains a comma (for tuples)
#         if ',' in value:
...
#         key, single_value, multi_value = match
#         key = key.replace('`', '')

#         if single_value:
#             value = single_value.replace('`', '')
#             if ',' in value:
#                 value = [v.strip() for v in value.split(',')]
#             else:
#                 value = [value]
#         elif multi_value:
#             value = [v.strip() for v in re.split(r'\s*\|\s*', multi_value)]
...
#   - **`dtype`**: `dtypes.void`
#   - **`src`**: `Tuple[()]`
#   - **`arg`**: `MetaOps.CUSTOM | MetaOps.COPY | MetaOps.EMPTY | MetaOps.VIEW`
# """
# x = UOp(UOps.ALU, dtypes.void, (), MetaOps.CUSTOM)
# types = parse_markdown(test_str3)
# # print(types)
# # print(x.__getattribute__("dtype"))
# # print(x.dtype)
# print("Types", types)
# for attr, typ in types.items():
#   print(attr, eval(typ))
#   # check_type(x.__getattribute__(attr), exec(typ))
#   # check_type(x.__getattribute__(attr), exec(typ))
#   print(x.__getattribute__(attr) == eval(typ))

# that's how we have to write Tuple
# check_type(x.src, Tuple[()])
